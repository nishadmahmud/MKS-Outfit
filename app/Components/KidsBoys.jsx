...
const KidsBoys = () => {
   ...
   const { data: kidsboys, isLoading } = useSWR(
    `${process.env.NEXT_PUBLIC_API}/public/subcategorywise-products/7277`,
    fetcher,
   
  );

  
//   console.log(kidsboys, 'data from men collection');
   
  return (
    <div className='w-11/12 mx-auto mt-12'>

     <div className='mb-5 flex items-center justify-between'>
       <Title title="Kids zone (Boys)"></Title>
 <Link href={`/subcategory/7277?subcategory=Boys&categoryId=7348`} className='hover:underline text-black'>View all</Link>
      ...
      {/* Products grid */}
        <div className="w-full grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-5">
          ...
          ) : kidsboys?.data.length > 0 ? (
            kidsboys?.data.slice(0, 4).map((product, idx) => (
