...
const KidsGirls = () => {
   ...
   const { data: kidsgirls, isLoading } = useSWR(
    `${process.env.NEXT_PUBLIC_API}/public/subcategorywise-products/7278`,
    fetcher,
   
  );

  
//   console.log(kidsgirls, 'data from men collection');
   
  return (
    <div className='w-11/12 mx-auto mt-12'>

     <div className='mb-5 flex items-center justify-between'>
       <Title title="Kids zone (Girls)"></Title>
 <Link href={`/subcategory/7278?subcategory=Girls&categoryId=7348`} className='hover:underline text-black'>View all</Link>
      ...
      {/* Products grid */}
        <div className="w-full grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-5">
          ...
          ) : kidsgirls?.data.length > 0 ? (
            kidsgirls?.data.slice(0, 4).map((product, idx) => (
